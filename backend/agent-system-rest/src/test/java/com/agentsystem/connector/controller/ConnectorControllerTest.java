...
class ConnectorControllerTest {
    ...
    void authUrl_returnsAuthUrl() {
        ...
        when(request.getAttribute("authenticatedUserUuid")).thenReturn("test-uuid");
        ...
        when(connectorService.getAuthUrl("google", "test-uuid", null))
    ...
    void status_returnsConnectionStatus() {
        ...
        when(request.getAttribute("authenticatedMode")).thenReturn("PERSONAL");
        when(request.getAttribute("authenticatedOrgId")).thenReturn(null);
        when(connectorService.getStatus("test-uuid", null))
    ...
    void disconnect_returns204() {
        ...
        verify(connectorService).disconnect("google", "test-uuid", null);
    }
    ...
    void telegramConnect_success_returns200() {
        ...
        assertThat(resp.getStatusCode().value()).isEqualTo(200);
        verify(telegramService).validateAndConnect(authData, "test-uuid", null);
    }
    ...
    void createGoogleDoc_success_returnsUrl() {
        ...
        when(request.getAttribute("authenticatedMode")).thenReturn("PERSONAL");
        when(request.getAttribute("authenticatedOrgId")).thenReturn(null);
        when(googleDocsService.createDocument("Test Doc", "Hello world", "test-uuid", null))
    ...
    void listCalendarEvents_returnsEventsString() {
        ...
        when(request.getAttribute("authenticatedMode")).thenReturn("PERSONAL");
        when(request.getAttribute("authenticatedOrgId")).thenReturn(null);
        when(googleCalendarService.listEvents("test-uuid", null, 10))
    ...
    void createCalendarEvent_success_returns200() {
        ...
        when(googleCalendarService.createEvent(
                eq("test-uuid"), isNull(), eq("Meeting"), eq("2024-01-01T10:00:00"),
