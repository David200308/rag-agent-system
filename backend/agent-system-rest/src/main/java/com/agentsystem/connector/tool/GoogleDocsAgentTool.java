 ...
 *
 * The tool is stateless; the current user's user_uuid is injected per-request via
 * {@link #setCurrentUserUuid(String)} before the ChatClient call and cleared
 * immediately after via {@link #clearCurrentUserUuid()}.
 *
...
public class GoogleDocsAgentTool {
    ...
    public String writeToGoogleDocs(String title, String content) {
        ...
        log.info("[GoogleDocsAgentTool] Creating doc '{}' for user '{}'", title, uuid);
        try {
            String url = googleDocsService.createDocument(title, content, uuid, CURRENT_ORG_ID.get());
    ...
    public String readGoogleDoc(String docUrl) {
        if (!toolCallBudget.tryConsume()) return ToolCallBudget.EXHAUSTED_MESSAGE;
        String uuid = CURRENT_USER_UUID.get();
        log.info("[GoogleDocsAgentTool] Reading doc '{}' for user '{}'", docUrl, uuid);
        try {
            String content = googleDocsService.readDocument(docUrl, uuid, CURRENT_ORG_ID.get());
